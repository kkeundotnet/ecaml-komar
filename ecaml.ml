...
let template_of_string s =
  let code_fragment =
    ...
    >>> ( p_str "raw"
        >>> p_str_until (p_str "%>")
        >>= (fun (expr, _) -> return (BlockOfRawExpr expr))
        ||| ( p_str_until (p_str "%>") >>= fun (expr, _) ->
              return (BlockOfExpr expr) ) )
    ||| ( p_str "<%" >>> p_str_until (p_str "%>") >>= fun (code, _) ->
          return (BlockOfCode code) )
    ||| ( check_prev (p_char '\n')
        >>> return ()
        ||| (p_pos 0 >>> return ())
        >>> p_many p_whitespace >>> p_char '%'
        >>> p_str_until (p_str "\n")
        >>= fun (code, _) -> return (LineOfCode code) )
  in
  let rec loop acc =
    let text_then_code =
      p_str_until code_fragment >>= fun (text, code) ->
      (* FIXME *)
      if text = "" then loop (code :: acc) else loop (code :: Text text :: acc)
    in
    (* no more code - return the rest *)
    let rest (s, pos) =
      let text = String.slice ~first:pos s in
      let rev_r = if text = "" then acc else Text text :: acc in
      return (List.rev rev_r) (s, String.length s)
    in
    text_then_code ||| rest
  in
